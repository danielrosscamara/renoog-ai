...
export const CreateUniversePage: React.FC<CreateUniversePageProps> = ({
  ...
  const [characterLocationMap, setCharacterLocationMap] = useState<Record<string, string>>({});
  ...
  const defaultSpawnLocationId =
    selectedWorld.default_location_id || selectedWorld.starter_locations[0]?.id || 'loc_hub';
  const effectivePlayerLocationId =
    playerLocationId && selectedWorld.starter_locations.some((l) => l.id === playerLocationId)
      ? playerLocationId
      : defaultSpawnLocationId;

  const getCompanionLocationId = (charId: string) => {
    const locId = characterLocationMap[charId];
    if (locId && selectedWorld.starter_locations.some((l) => l.id === locId)) {
      return locId;
    }
    return defaultSpawnLocationId;
  };
  ...
  const spawnLocation =
    selectedWorld.starter_locations.find(
      (loc) => loc.id === effectivePlayerLocationId
      ...
      <div className="max-w-6xl w-full mx-auto p-6 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-2.5">
              {selectedCharacters.length === 0 ? (
                ...
                selectedCharacters.map((c) => {
                  const assignedLocationId = getCompanionLocationId(c.id);
                  return (
                    <div
                      key={c.id}
                      className="p-4 rounded-2xl bg-zinc-900/50 border border-zinc-800/80 flex flex-col sm:flex-row sm:items-center gap-4 group hover:border-zinc-700 transition-colors"
                    >
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Companion Starting Room Selector */}
                          <div className="flex items-center gap-1.5 mt-2">
                            <MapPin className="w-3 h-3 text-indigo-400 shrink-0" />
                            <span className="text-[11px] text-zinc-400 font-medium shrink-0">Spawn in:</span>
                            <select
                              value={assignedLocationId}
                              onChange={(e) => {
                                const newLoc = e.target.value;
                                setCharacterLocationMap((prev) => ({
                                  ...prev,
                                  [c.id]: newLoc,
                                }));
                              }}
                              className="px-2 py-1 rounded-lg bg-zinc-950 border border-zinc-800 hover:border-zinc-700 text-xs text-zinc-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 outline-none cursor-pointer max-w-56 truncate"
                            >
                              {selectedWorld.starter_locations.map((loc) => {
                                const isDefault = loc.id === defaultSpawnLocationId;
                                return (
                                  <option key={loc.id} value={loc.id} className="bg-zinc-900 text-zinc-200">
                                    {loc.name} {isDefault ? '(Spawn Hub)' : ''}
                                  </option>
                                );
                              })}
                            </select>
                      ...
                      <button
                        ...
                        className="self-end sm:self-center p-1.5 rounded-lg text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer"
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 space-y-5 p-6 rounded-3xl bg-zinc-900/60 border border-zinc-800/80 shadow-xl backdrop-blur-sm">
            ...
            {/* PLAYER STARTING LOCATION SELECTOR */}
            <div className="space-y-1.5 pt-3 border-t border-zinc-800/60">
              <label className="text-[11px] font-semibold text-zinc-300 flex items-center justify-between">
                <span>Player Starting Location</span>
                <span className="text-[10px] text-zinc-500">Where you arrive</span>
              </label>
              <select
                value={effectivePlayerLocationId}
                onChange={(e) => setPlayerLocationId(e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl bg-zinc-950 border border-zinc-800 hover:border-zinc-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 text-xs text-white outline-none cursor-pointer transition-colors"
              >
                {selectedWorld.starter_locations.map((loc) => {
                  const isDefault = loc.id === defaultSpawnLocationId;
                  return (
            ...
            <div className="space-y-2 pt-3 border-t border-zinc-800/60">
              ...
              <ul className="space-y-2 text-xs text-zinc-400">
                <li className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-zinc-400 bg-zinc-950/60 px-2 py-0.5 rounded border border-zinc-800/80">
                    Omnipresent
                  </span>
                </li>
                {selectedCharacters.map((c) => {
                  const charLocId = getCompanionLocationId(c.id);
                  const charLocName =
                    selectedWorld.starter_locations.find((l) => l.id === charLocId)?.name || 'Spawn Hub';
                  return (
                    <li key={c.id} className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-indigo-300 bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20 truncate max-w-28 shrink-0 ml-2">
                        {charLocName}
                      </span>
                    </li>
                  );
                ...
                <li className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20 truncate max-w-28 shrink-0 ml-2">
                    {spawnLocation?.name || 'Spawn Hub'}
                  </span>
                </li>
              </ul>
            </div>
